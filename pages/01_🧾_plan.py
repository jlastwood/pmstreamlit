...
st.set_page_config(
      page_title="The PM Monitor Plan form",
      page_icon=im,
      layout="wide",
...
with st.form(key="pmmonitorplan", clear_on_submit=False):
     tab1, tab2, tab3, tab4, tab5, tab6, tab7, tab8, tab9 = st.tabs(["Info", "Scope", "Cost", "Schedule", "Communication", "Quality", "Constraints", "ROI", "Save"])
     ...
     with tab1:
      ...
      st.session_state['plspurpose']  = st.text_input ("What is the purpose?", value=setvalue('plspurpose'))
      st.session_state['plsbenefits']  = st.text_input ("What are the Benefits?", value=setvalue('plsbenefits'))
      st.session_state['plsbenchmarks']  = st.text_input ("Are there comparable benchmarks or services?", value=setvalue('plsbenchmarks'))
     ...
     # how much time to end of plan
      ...
      with col7:
       phasechoice = {"None", "Initiation", "Planning", "Execution", "Closure"}
       phaselist = ["None", "Initiation", "Planning", "Execution", "Closure"]
      ...
      with col8:
       classchoice = pd.DataFrame({'Item': ['None', 'Software Build', 'Software Design', 'Brand Marketing', 'Process Automation', 'Scheduled Maintenance'], 'Value': [0, 1, 2, 3, 4, 5]})
     ...
     with tab2:
      ...
      with col4:
       st.session_state['plmlistscopelist'] = st.multiselect(
         ...
         default=setvalue('plmlistscoplist'))
      with col5:
      ...
      st.session_state['thepmplanscope'] = "The scope includes " + st.session_state['plscopenicetohave'] + " and " + st.session_state['plscopeifpossible'] + "<br>The following must be delivered as part of scope. " + st.session_state['plscopemusthave'] + " and the following are out of scope " + st.session_state['plscopeoutofscope'] 
      st.write(st.session_state['thepmplanscope'])
      if st.session_state['plscopechangeadd'] == "None" and st.session_state['plscopechangedel'] == "None":
     ...
     with tab5:
      ...
      with col3:
       st.session_state['plnusers']  = st.slider ("Number of Users", value=setvalue('plnusers'), format="%i", min_value=0, max_value=1000000, step=10)
      with col4:
       st.session_state['plnactors']  = st.slider ("Number of Actors", value=setvalue('plnactors'), format="%i", min_value=0, max_value=10, step=1)
      col1, col2, col3, col4 = st.columns(4)
      with col1:
       st.session_state['plnteamweeks']  = st.slider ("Weeks with Full Team", value=setvalue('plnteamweeks'), format="%i", min_value=0, max_value=52, step=1 )
      with col2:
      ...
      with col3:
       st.session_state['plpactivitylink']  = st.text_input ("Activity Source Link (URL)", value=setvalue('plpactivitlink') )
      with col4:
      ...
      planchannels = str((st.session_state['plnstakeholder'] + st.session_state['plnteam']) * (st.session_state['plnstakeholder'] + st.session_state['plnteam'] - 1))
      ...
      plancommunication = "There are " + planchannels + " communication channels " 
      st.session_state['thepmteam'] = "The team is composed of " + str(st.session_state['plnteam']) + " members and have collaborated together for " + str(st.session_state['plnteamweeks']) + "<br> The solution architect is " + st.session_state['plpsolutionname'] + ".  The Operations Lead is " + st.session_state['plpoperationname'] + plancommunication
     ...
     #  test results, test not done, test run less than tolerance of 70% test failed more than 20 or critical test failed is positive
      ...
      planquality = "There are " + str(st.session_state['plntests']) + " tests planned. " 
      passfail = 0
      passcoverage = 0
      ...
      st.session_state['thepmquality'] = " " + planquality + str(st.session_state['plnteam']) + " members and have collaborated together for " + str(st.session_state['plnteamweeks']) + " weeks. The inspector team/name is " + st.session_state['plpinspectorname'] + ". The inspection report is due " + st.session_state['pldinspectdate'].strftime('%Y-%m-%d') + ".  Pass to fail rate is " + str(passfail) + "%. Test coverage is " + str(passcoverage) + "%."  
      st.markdown('##') 
      st.write(st.session_state['thepmquality'])
      st.write(st.session_state['thepminspectionwarning'])
     ...
     with tab3:
      ...
      (evsummary, st.session_state['thepmcpi'], st.session_state['thepmspi'], etc)  = evreport((plbudget + (plbudget *.3)), plbudget, st.session_state['plnhours'], st.session_state['plnavgrate'], "EUR" , st.session_state['pldstartdate'], st.session_state['pldenddate'], st.session_state['plnspend'], mplanned, mcomplete, daystoend, daystoday, st.session_state['thepmtimecomplete'])
